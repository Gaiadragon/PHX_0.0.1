...
remote.addinterface("treefarm_interface",
{
  addSeed = function(seedInfo)
    ...
    if glob.tf.seedPrototypes[seedInfo.name] == nil then
      glob.tf.seedPrototypes[seedInfo.name] = {}
      if seedInfo.states ~= nil then
        glob.tf.seedPrototypes[seedInfo.name].states = seedInfo.states
      else
        return "growing states not defined"
      end
      if seedInfo.output ~= nil then
        glob.tf.seedPrototypes[seedInfo.name].output = seedInfo.output
      else
        return "result not defined"
      end
      if seedInfo.efficiency then
        glob.tf.seedPrototypes[seedInfo.name].efficiency = seedInfo.efficiency
        if glob.tf.seedPrototypes[seedInfo.name].efficiency.other == 0 then
          glob.tf.seedPrototypes[seedInfo.name].efficiency.other = 0.01
        end
      else
        return "efficiency not defined"
      end
      if seedInfo.basicGrowingTime ~= nil then
        glob.tf.seedPrototypes[seedInfo.name].basicGrowingTime = seedInfo.basicGrowingTime
      else
        return "basicGrowingTime not defined"
      end
      if seedInfo.randomGrowingTime ~= nil then     
        glob.tf.seedPrototypes[seedInfo.name].randomGrowingTime = seedInfo.randomGrowingTime
      else
        return "randomGrowingTime not defined"
      end
      if seedInfo.fertilizerBoost ~= nil then
        glob.tf.seedPrototypes[seedInfo.name].fertilizerBoost = seedInfo.fertilizerBoost
      else
  ...
  readSeed = function(seedName)
    return glob.tf.seedPrototypes[seedName]
  end,

  getSeedTypesData = function()
    return glob.tf.seedPrototypes
  end,


  getNumGrowing = function()
    return #glob.tf.growing
  end,

  getFirstPlantTick = function()
    return glob.tf.growing[1].nextUpdate
  end,

  removeAllPlants = function()
    for _, plant in ipairs(glob.tf.growing) do
      if plant.entity.valid then
        plant.entity.destroy()
      end
    end

    while (#glob.tf.growing > 0) do
      table.remove(glob.tf.growing)
    end
  end,
